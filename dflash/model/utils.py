...
def load_and_process_dataset(data_name: str):
    # Math datasets
    if data_name == "gsm8k":
        dataset = load_dataset("gsm8k", "main", split="test") # 1.3k左右数据
        prompt_fmt = "{question}\nPlease reason step by step, and put your final answer within \\boxed{{}}."
        dataset = dataset.map(lambda x: {"turns": [prompt_fmt.format(**x)]})

    elif data_name == "mgsm_zh":
        tsv_path = "/mnt/shared-storage-user/leihaodi/data/mgsm/mgsm_zh.tsv"
        prompt_fmt = "{question}\n请逐步推理分析来解答问题，并把最终答案放在 \\boxed{{}} 中。"
        rows = []
                ...
                rows.append(
                    {
                        "question": question,
                        "answer": answer,
                        "turns": [prompt_fmt.format(question=question)],
                    }
                )
        dataset = Dataset.from_list(rows)

    elif data_name == "acp_app_bool":
        dataset = load_dataset("ibm-research/acp_bench", "acp_app_bool", split="test")
        prompt_fmt = (
    ...
    elif data_name == "acp_app_gen":
        dataset = load_dataset("ibm-research/acp_bench", "acp_app_gen", split="test")
        prompt_fmt = (
            "Known information: {context}\nQuestion: {question}. "
            "Please reason step by step, and put your final answer within \\boxed{{}}."
        )
