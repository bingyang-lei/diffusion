...
def _record_cuda_elapsed_ms(
    profile_stats: Optional[dict[str, Any]],
    total_key: str,
    count_key: str,
    start_event: Optional[torch.cuda.Event],
    end_event: Optional[torch.cuda.Event],
) -> None:
    if profile_stats is None or start_event is None or end_event is None:
        return
    pending_events = profile_stats.setdefault("_pending_cuda_events", [])
    pending_events.append((total_key, count_key, start_event, end_event))


def _cuda_event_pair(enabled: bool) -> tuple[Optional[torch.cuda.Event], Optional[torch.cuda.Event]]:
    if not enabled:
        return None, None
...
class Qwen3DFlashDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        layer_start_event, layer_end_event = _cuda_event_pair(profile_stats is not None)
        if layer_start_event is not None:
            layer_start_event.record()

        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)
        attn_start_event, attn_end_event = _cuda_event_pair(profile_stats is not None)
        ...
        mlp_start_event, mlp_end_event = _cuda_event_pair(profile_stats is not None)
        ...
        _record_cuda_elapsed_ms(profile_stats, "layer_total_ms", "layer_total_count", layer_start_event, layer_end_event)
        _record_cuda_elapsed_ms(profile_stats, "self_attn_ms", "self_attn_count", attn_start_event, attn_end_event)
        _record_cuda_elapsed_ms(profile_stats, "mlp_ms", "mlp_count", mlp_start_event, mlp_end_event)
